...
showhelp () {
    ...
    echo "-v <rpm_version> (Optional) The version of the rpm to manage. Default: installed"
...
do_install () {
    ...
    if [ "$?" = "0" ]; then
        ...
        if [ $archivedir/$project/installed ] ; then
            rm -f $archivedir/$project/installed
        fi
        ln -sf $archivedir/$project/$oldversion $archivedir/$project/installed
    fi
